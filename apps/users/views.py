...
from .forms import LoginForm, RegisterForm, ForgetForm, ModifyPwdForm, UploadImageForm, UploadInfoForm # form表单
...
class CustomBackend(ModelBackend):
    def authenticate(self, username=None, password=None, **kwargs):
        try:
            ...
            # 后台为密码密文，传入的为密码明文， 使用check_password()方法验证密码
        ...
        if pre_check:
            ...
            if user is not None:      # 如果成功返回 对象，失败返回 None
                if user.is_active:
                    ...
                    return render(request, "login.html", {"msg": u"请前往您的邮箱验证"})
            else:
                # 登陆失败，给出提示信息
...
class ForgetPwdView(View):
    ...
    def post(self, request):
        ...
        pre_check = forget_form.is_valid()
        if pre_check:
...
class ModifyPwdView(View):
    ...
    def post(self, request):
        ...
        pre_check = modify_form.is_valid()
        if pre_check:
...
class UploadImageView(LoginRequiredMixin, View):
    # 用户修改头像
    def post(self, request):
        # 利用model form 完成有model和form的特性， 直接保存
        image_form = UploadImageForm(request.POST, request.FILES, instance=request.user)
        if image_form.is_valid():
            # form保存
            #image = image_form.cleaned_data['image']
            #request.user.image = image_form
            #request.user.save()
