...
class AddUserAskView(View):
    ...
    def post(self, request):
        ...
        pre_check = userask_form.is_valid()
        if pre_check:

            user_ask = userask_form.save(commit=True)

            # 异步的操作。  Json字符串,content_type='application/json' 浏览器解释为json
...
class OrgHomeView(View):
    ...
    def get(self, request, org_id):
        ...
        all_courses = course_org.course_set.all()[:3]  # 通过课程反向取课程机构
...
class OrgCourseView(View):
    ...
    def get(self, request, org_id):
        ...
        if request.user.is_authenticated():
            if UserFavorite.objects.filter(user=request.user, fav_id=course_org.id, fav_type=2):
                has_fav = True
        all_courses = course_org.course_set.all()  # 通过课程反向取课程机构
...
class OrgTeacherView(View):
    ...
    def get(self, request, org_id):
        ...
        if request.user.is_authenticated():
            if UserFavorite.objects.filter(user=request.user, fav_id=course_org.id, fav_type=2):
                has_fav = True
        all_teachers = course_org.teacher_set.all()  # 通过课程反向取课程机构
